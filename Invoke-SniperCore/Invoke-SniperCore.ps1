    ...
    process{
        if(-not($NmapXML)){
            ...
            if($ExcludeComputerList){
                nmap -sS -sU -T4 -n -v --open -oA $output/nmap-overall --min-parallelism $min_paralell --min-hostgroup $min_hostgroup -p T:1-65535,U:$top1000 -iL $ComputerList --excludefile $ExcludeComputerList
            }else{
                nmap -sS -sU -T4 -n -v --open -oA $output/nmap-overall --min-parallelism $min_paralell --min-hostgroup $min_hostgroup -p T:1-65535,U:$top1000 -iL $ComputerList
            }
        }
        ...
        foreach($machine in (Get-ChildItem $output\machines\)) {
            ...
            Write-Output "`n[*]Scanning $computer"
            ...
            nmap -sS -sU -sC -sV -v -T4 -n -Pn -p T:$tcp_ports,U:$udp_ports --script=$nmap_script -oA $path/nmap-scriptscan $computer
            if((Get-ChildItem $path/nmap-scriptscan.gnmap).length -lt 1000){
                Write-Output "[-] Segmentation fault may occurred on $computer"
                Add-Content -Value "$computer" -Path $output/error.txt
            }
            Write-Output "`n[*] Creating Searchsploit Report"
            searchsploit -v --nmap $path/nmap-scriptscan.xml 2>&1 > $path/searchsploit.txt
        }
    }
}